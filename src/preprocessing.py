...
def max_min_difference_sensors(df):
    ...
    for i in range(1, 101):
        ...
        for sensor in important_sensor:
            ...
            sensor_difference = round(engine[sensor].max() - engine[sensor].min(),2)
...
#for testing data with LSTM
def null_preprocess(df_test):
     df_test.fillna(df_test.mean(numeric_only = True), inplace= True)
     return df_test



def max_min_difference_sensors(df_test):
    ...
    for i in range(1, 101):
        ...
        engine = df_test[df_test["engine_id"] == i]
...
def rolling_mean(df_test, sensors, window):

    df_test = df_test.copy()

    for sensor in sensors:

        df_test[f"{sensor}_rolling_mean"] = np.nan

        for engine_id in df_test["engine_id"].unique():

            engine = df_test[df_test["engine_id"] == engine_id]
            ...
            df_test.loc[engine.index, f"{sensor}_rolling_mean"] = rolling_values

    return df_test


def rolling_std(df_test, sensor, window=5):
    ...
    for sensor in sensor:
        ...
        df_test[f"{sensor}_rollstd_{window}"] = rolling_values
